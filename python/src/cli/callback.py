...
def callback(
	...
	try:
		...
		log.error('Callback timeout', url=url)
		typer.echo('Error: callback request timeout', err=True)
		raise typer.Exit(1)
	except httpx.ConnectError as e:
		log.error('Callback connect error', url=url, error=str(e)[:80])
		typer.echo(f'Error: {e}', err=True)
		raise typer.Exit(1)

	typer.echo(resp.text)
	if resp.status_code != 200:
		log.warning('Callback non-200', status=resp.status_code)
		raise typer.Exit(1)
	log.info('Callback success', status=resp.status_code)
