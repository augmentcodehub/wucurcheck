...
def run():
    ...
    print(f"[INFO] sign_in: {sign_in_url}, user_info: {user_info_url}")
    ...
    for i, acct in enumerate(accounts):
        ...
        # Each account gets its own client (fresh session)
        with httpx.Client(http2=True, timeout=30.0) as client:
            try:
                # Login (sets session cookie on client)
