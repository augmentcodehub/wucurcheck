...
try:
	from .account_registry_db import DEFAULT_DB_PATH, connect_db, ensure_schema, upsert_registered_account
	from .register_one_account import load_account_from_file, load_account_from_json_text, validate_account
...
def run_register_json(account: dict[str, str], skip_checkin: bool) -> tuple[int, dict | None]:
	...
	print('[INFO] Registering single account and writing result to SQLite')
	...
	try:
		...
		print(f'[FAILED] Could not parse JSON result from register_wucur.py: {exc}')
		return 1, None
...
def main(argv: list[str] | None = None) -> int:
	...
	if exit_code != 0 or payload is None:
		print(f'[FAILED] Registration flow failed with exit code {exit_code}')
		return exit_code or 1

	try:
		persist_success(validated, payload, Path(args.db))
	except Exception as exc:
		print(f'[FAILED] Registration succeeded but SQLite persistence failed: {exc}')
		return 1
