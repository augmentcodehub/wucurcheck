...
import { TempEmailService } from './email/email-service'
...
export class Registrar {
  ...
  /** 设置邮箱服务 */
  setEmailService(svc: TempEmailService): void {
    this.emailSvc = svc
  }
  ...
  private get sessionOpts() {
    const proxyUrl = this.cfg.proxy || process.env.HTTPS_PROXY || process.env.https_proxy || process.env.HTTP_PROXY || process.env.http_proxy || undefined
    return {
  ...
  private ensureTlsLibInTmpDir(): void {
    ...
    const fs = require('fs')
    const filename = 'tls-client-xgo-1.7.9-linux-amd64.so'
    ...
    if (fs.existsSync(libPath)) {
      this.log('[TLS] Copying library from lib/ to tmpdir: ' + libPath + ' -> ' + tmpPath)
      fs.copyFileSync(libPath, tmpPath)
      return
    }
    this.log('[TLS] Library not found at: ' + libPath)
  }
  ...
  private async step3Email(): Promise<void> {
    ...
    if (!this.emailSvc) throw new Error('emailSvc 未设置')
    this.log('[3] 创建邮箱')
    this.email = await this.emailSvc.create()
    if (!this.email) throw new Error('创建邮箱失败')
